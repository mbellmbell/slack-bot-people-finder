...
let parseCompanyFromMessage = (msg) => {
  ...
  try {
    companies.push({
      ...
      type: regex_result[1].trim(),
      domain: domain,
      new: true
    })
  } catch (err) {
    sendMessage(`Can't parse company`)
  }
}
      ...
      if (!c.requesting) {
        ...
        request.get(process.env.EMAILFINDER_URL + 'api/find?' + qs.stringify({
          ...
          if (body.people.length > 0) {
            ...
            body.people.forEach((p, index) => template += ` > ${p.name} | ${p.title} | ${p.email||'_(No email found)_'} _${p.confidence ? parseInt(p.confidence*100) + '% confidence' : ' '}_\n`)
          } else {
