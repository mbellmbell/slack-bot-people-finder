...
let handleCompanies = async() => {
  let handle = () => new Promise(async resolve => {
    console.log('handleCompanies')

    items = items.filter(c => !c.finished)

    items = items.map(async c => {
      if (c.new) {
      ...
      if (!c.requesting) {
        ...
        if (c.type === 'company') {
            ...
            await new Promise((resolve) => {
              request.get({
                ...
                if (body.people.length > 0) {
                  ...
                  body.people.forEach((p, index) => template += ` > ${p.name} ${p.title ? '| ' +p.title + ' ' : ''}| ${p.email||'_(No email found)_'} ${p.confidence ? '_'+parseInt(p.confidence*100) + '% confidence_' : ''}\n`)
                } else {
          ...
          if (error) {
            sendMessage(`Error getting ${c.company} information ${error}`)
          }
        }
        c.finished = true
      }
